...
col_types_spec <- cols(
  zg_GEOID10 = col_character(),
  zg_GEOID = col_character(),
  zg_COUNTYFP = col_character(),
  zg_ZCTA5CE10 = col_character(),
...
process_file <- function(file) {
  read_csv(file, col_types = col_types_spec) %>%
    select(precinct_unique_id, precinct_area, starts_with('zg_'), starts_with('G16'),
...
intersections2 <- intersections %>% rename(
  zcta_GEOID = zg_GEOID10,
  zcta_ALAND = zg_ALAND10,
  zcta_AWATER= zg_AWATER10,
  bg_GEOID = zg_GEOID,
  bg_ALAND = zg_ALAND,
  bg_AWATER= zg_AWATER,
) %>% mutate(
...
acs_zcta <- read_csv( '/Users/amir/Downloads/Data/ACS-ZCTA/ACSDP5Y2016.DP05-Data.csv' ) %>%
  slice(-1) %>%
...
acs_zcta_hh <- read_csv( 'data/household-pop-zcta/ACSDT5Y2016.B11002-Data.csv' ) %>%
  slice(-1) %>%
  mutate(zcta_GEOID = str_sub(GEO_ID, 10)) %>%
  select(
    zcta_GEOID,
...
remove(intersections, intersections2, acs_zcta, acs_zcta2, acs_zcta_hh, acs_bg_hh)
...
write_rds(df_dist_precinct, "zcta-before-agg-2016.rds")
df_dist_precinct <- read_rds("zcta-before-agg-2016.rds")
...
df_weight1_applied <- df_dist_precinct %>% mutate(
    across(starts_with('G16'), ~ .x * votes_allocation_weight1)
)
...
fwrite(
  votes_zcta_agg1, 
  file = "REVISE/produced-data/election-results-zcta-2016-areal-method.csv", 
...
remove(df_weight1_applied)


# imperviousness
df_weight2_applied <- df_dist_precinct %>% mutate(
  across(starts_with('G16'), ~ .x * votes_allocation_weight2)
)
...
fwrite(
  votes_zcta_agg2, 
  file = "REVISE/produced-data/election-results-zcta-2016-imperviousness-method.csv", 
  sep = ",",          # Use a comma as the separator
  quote = TRUE,       # Quote character strings to handle special characters
  bom = TRUE,         # Add BOM for compatibility with Excel
  encoding = "UTF-8"  # Ensure UTF-8 encoding
...
remove(df_weight2_applied)


# land cover
df_weight3_applied <- df_dist_precinct %>% mutate(
  across(starts_with('G16'), ~ .x * votes_allocation_weight3)
)
...
fwrite(
  votes_zcta_agg3, 
  file = "REVISE/produced-data/election-results-zcta-2016-landcover-method.csv", 
  sep = ",",          # Use a comma as the separator
  quote = TRUE,       # Quote character strings to handle special characters
  bom = TRUE,         # Add BOM for compatibility with Excel
  encoding = "UTF-8"  # Ensure UTF-8 encoding
